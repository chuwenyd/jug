...
def display_results(placement_results, salary_results, salary_binary_results, data):
    ...
    # 图片5：就业类别分布图 (Class Imbalance Plot)
    # 计算原始数据中的就业状态分布
    original_counts = data['placement']['y_train'].value_counts()
    
    # 使用SMOTE处理就业预测数据
    smote = SMOTE(random_state=42)
    X_train_resampled, y_train_resampled = smote.fit_resample(data['placement']['X_train'], data['placement']['y_train'])
    ...
    # 绘制原始分布
    plt.bar([0, 1], original_counts, width=0.4, label='原始数据', color='skyblue', align='center')
    
    # 绘制SMOTE处理后的分布
    plt.bar([2, 3], smote_counts, width=0.4, label='SMOTE处理后', color='salmon', align='center')
    
    # 设置x轴标签位置
    plt.xticks([0.5, 2.5], ['原始数据 (2:1)', 'SMOTE处理后 (1:1)'])
    ...
    for i, v in enumerate(smote_counts):
        plt.text(i + 2, v + 5, str(v), ha='center', va='bottom')
    
    plt.title('就业类别分布图')
    plt.ylabel('学生人数')
    plt.legend()
    plt.tight_layout()
    plt.savefig('employment_class_imbalance.png')
    plt.close()
    
    # 图片6：薪资分布直方图 (Salary Distribution Histogram)
    # 获取原始数据中的薪资信息
    df_original = pd.read_csv('Placement_Data_Full_Class.csv')
    df_placed = df_original[df_original['status'] == 'Placed']
    
    plt.figure(figsize=(12, 8))
    sns.histplot(df_placed['salary'], kde=True, bins=20, color='skyblue')
    
    # 添加中位数虚线
    median_val = df_placed['salary'].median()
    plt.axvline(median_val, color='red', linestyle='--', label=f'中位数阈值: {median_val:.0f}')
    
    plt.title('已就业学生薪资分布直方图')
    plt.xlabel('薪资')
    plt.ylabel('学生人数')
    plt.legend()
    plt.tight_layout()
...
def main():
    ...
    print("- employment_class_imbalance.png: 就业类别分布图")
    print("- salary_distribution_histogram.png: 薪资分布直方图")
